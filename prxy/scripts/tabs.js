let allCloseTabBtns = document.querySelectorAll('.close-tab');
        ...
        document.querySelector(".fullscreen-sj-frame-btn").addEventListener("click", () => {
            active().frame.requestFullscreen();
        })

        document.querySelector(".devtools-icon").addEventListener("click", () => {
            let cdoc = active().frame.contentDocument
            let cwin = active().frame.contentWindow
            let s = cdoc.createElement('script')
...
*/
        ...
        function createTab() {
            ...
            frame.addEventListener("urlchange", (e) => {
                ...
                //Above function is just to stop the loading bar

                //set bg to white so websites with no bg dont look wonky donkey 🐴 🐪 🐎
                //Uses frame and not frame.frame b/c when it is called frame has already bean set to frame.frame
                /*
                Could probally just use a different variable to make less confusing but IDC
                */
               
                frame.style.background = 'rgb(255, 255, 255)'
            })
        ...
        frame.addEventListener("load", (e) => {
            ...
            */
            ...
            //focus the tab
            focusTab(tabId)
            document.querySelector(".sj-input").foc
        }
